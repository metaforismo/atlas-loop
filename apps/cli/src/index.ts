...
type SessionHandoffBundleFileKey = keyof SessionHandoffBundleFiles;
type SessionHandoffBundleIntegrityKey = keyof SessionHandoffBundleIntegrity;

interface HandoffVerifyIssue {
  severity: "error" | "warning";
  path: string;
  message: string;
}

interface SessionHandoffBundleVerification {
  ok: boolean;
...
export async function verifySessionHandoffBundle(params: { bundleDir: string }): Promise<SessionHandoffBundleVerification> {
  ...
  try {
    ...
    if (!bundleStats.isDirectory()) {
      addVerifyIssue(issues, bundleDir, "handoff bundle path must resolve to a directory");
      return buildHandoffVerificationResult({ bundleDir, manifestPath, manifest, checkedAt: new Date().toISOString(), filesChecked, issues });
    }
  } catch (error) {
    addVerifyIssue(issues, bundleDir, `handoff bundle directory could not be read: ${formatErrorMessage(error)}`);
    return buildHandoffVerificationResult({ bundleDir, manifestPath, manifest, checkedAt: new Date().toISOString(), filesChecked, issues });
  }
  ...
  try {
    ...
    manifest = recordValue(parsedManifest);
    if (!manifest) {
      addVerifyIssue(issues, manifestPath, "manifest.json must contain a JSON object");
      return buildHandoffVerificationResult({ bundleDir, manifestPath, manifest, checkedAt: new Date().toISOString(), filesChecked, issues });
    }
  } catch (error) {
    addVerifyIssue(issues, manifestPath, `manifest.json could not be read or parsed: ${formatErrorMessage(error)}`);
    return buildHandoffVerificationResult({ bundleDir, manifestPath, manifest, checkedAt: new Date().toISOString(), filesChecked, issues });
  }

  if (manifest.schemaVersion !== "atlas-loop.handoff-bundle.v1") {
    addVerifyIssue(issues, "schemaVersion", "schemaVersion must be atlas-loop.handoff-bundle.v1");
  }
  if (manifest.localOnly !== true) {
    addVerifyIssue(issues, "localOnly", "localOnly must be true");
  }
  if (manifest.uploaded !== false) {
    addVerifyIssue(issues, "uploaded", "uploaded must be false");
  }

  const files = recordValue(manifest.files);
  const integrity = recordValue(manifest.integrity);
  if (!files) addVerifyIssue(issues, "files", "manifest files must be an object");
  if (!integrity) addVerifyIssue(issues, "integrity", "manifest integrity must be an object");

  if (integrity) {
    if (hasOwn(integrity, "manifest")) {
      addVerifyIssue(issues, "integrity.manifest", "integrity must not include manifest");
    }
    for (const key of Object.keys(integrity).sort()) {
      if (key !== "manifest" && !isHandoffBundleIntegrityKey(key)) {
        addVerifyIssue(issues, `integrity.${key}`, "integrity contains an unknown file key");
      }
    }
  }
  ...
  for (const key of REQUIRED_HANDOFF_BUNDLE_FILE_KEYS) {
    ...
    if (key === "manifest") {
      if (resolvedPath && resolvedPath !== manifestPath) {
        addVerifyIssue(issues, String(files?.manifest ?? "files.manifest"), "files.manifest must point to the bundle manifest.json that was verified");
      }
      continue;
    }
  ...
  for (const key of OPTIONAL_HANDOFF_BUNDLE_FILE_KEYS) {
    ...
    if (!hasFilePath) {
      if (hasIntegrityEntry) {
        addVerifyIssue(issues, `integrity.${key}`, `integrity.${key} is present without files.${key}`);
      }
      continue;
    }
...
async function verifyHandoffBundleFileValue(params: {
  ...
  if (value === null || value === undefined) {
    if (params.required) {
      addVerifyIssue(params.issues, `files.${params.key}`, `files.${params.key} is required`);
    }
    return null;
  }
  if (value === "") {
    addVerifyIssue(params.issues, `files.${params.key}`, `files.${params.key} must be null or a non-empty file path string`);
    return null;
  }
  if (typeof value !== "string") {
    addVerifyIssue(params.issues, `files.${params.key}`, `files.${params.key} must be a file path string`);
    return null;
  }
  if (value.includes("://")) {
    addVerifyIssue(params.issues, value, `files.${params.key} must be a local filesystem path inside the bundle`);
    return null;
  }

  const resolvedPath = resolve(isAbsolute(value) ? value : join(params.bundleDir, value));
  if (!pathStaysInside(params.bundleDir, resolvedPath)) {
    addVerifyIssue(params.issues, value, `files.${params.key} must stay inside the bundle`);
    return null;
  }
  let fileStats;
  try {
    ...
    addVerifyIssue(params.issues, resolvedPath, `files.${params.key} does not exist: ${formatErrorMessage(error)}`);
    return null;
  }
  if (!fileStats.isFile()) {
    addVerifyIssue(params.issues, resolvedPath, `files.${params.key} must be a regular file inside the bundle`);
    return null;
  }
  ...
  try {
    ...
    addVerifyIssue(params.issues, resolvedPath, `files.${params.key} real path could not be read: ${formatErrorMessage(error)}`);
    return null;
  }
  if (!pathStaysInside(params.realBundleDir, realResolvedPath)) {
    addVerifyIssue(params.issues, resolvedPath, `files.${params.key} must resolve inside the bundle`);
    return null;
  }
...
function verifyHandoffBundleIntegrityEntry(params: {
  ...
  if (!params.integrity || !hasOwn(params.integrity, params.key)) {
    if (params.required) {
      addVerifyIssue(params.issues, `integrity.${params.key}`, `missing required integrity for files.${params.key}`);
    }
    return null;
  }

  const entry = recordValue(params.integrity[params.key]);
  if (!entry) {
    addVerifyIssue(params.issues, `integrity.${params.key}`, `integrity.${params.key} must be an object`);
    return null;
  }
  ...
  if (typeof sha256 !== "string" || !/^[a-f0-9]{64}$/.test(sha256)) {
    addVerifyIssue(params.issues, `integrity.${params.key}.sha256`, `integrity.${params.key}.sha256 must be a SHA-256 hex digest`);
    valid = false;
  }
  if (typeof sizeBytes !== "number" || !Number.isSafeInteger(sizeBytes) || sizeBytes < 0) {
    addVerifyIssue(params.issues, `integrity.${params.key}.sizeBytes`, `integrity.${params.key}.sizeBytes must be a non-negative integer`);
    valid = false;
  }
...
async function verifyHandoffBundleFileIntegrity(
  ...
  try {
    actualIntegrity = await readFileIntegrity(filePath);
  } catch (error) {
    addVerifyIssue(issues, filePath, `could not read file for integrity: ${formatErrorMessage(error)}`);
    return 0;
  }

  if (actualIntegrity.sha256 !== expectedIntegrity.sha256) {
    addVerifyIssue(issues, filePath, `sha256 mismatch: expected ${expectedIntegrity.sha256}, got ${actualIntegrity.sha256}`);
  }
  if (actualIntegrity.sizeBytes !== expectedIntegrity.sizeBytes) {
    addVerifyIssue(issues, filePath, `sizeBytes mismatch: expected ${expectedIntegrity.sizeBytes}, got ${actualIntegrity.sizeBytes}`);
  }
  return 1;
}
...
function addVerifyIssue(issues: HandoffVerifyIssue[], path: string, message: string, severity: HandoffVerifyIssue["severity"] = "error"): void {
...
function pathStaysInside(parentPath: string, childPath: string): boolean {
