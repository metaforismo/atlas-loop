...
- **Map real journeys** — derive screens and transitions from captured evidence, with deep links back to the producing session and action. Each run's walk through the app is reconstructed from the transitions it produced, listed by what it did (input, gesture, verified, navigate) rather than by guessed screen names, and walkable a step at a time with everything off the path dimmed. A walk is split where its chain breaks rather than joined across an edge nobody observed.
