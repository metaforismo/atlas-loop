...
| React viewer | Operational overview, deterministic local test compiler, first-class session triage with input-backend filters, searchable observed-app history, URL-backed test, session, app, and workflow workspaces, prefilled session launcher, keyboard command search, live device image, reusable multi-gesture workflows, observed-flow summary, timeline, evidence inspection, Atlas map, visual diffs, and handoff UI |
